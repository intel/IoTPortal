...
const DevicesCard = ({
  ...
  const deviceStatusItemTemplate = (option) => {
    return (
      <span
        className={classNames('device-badge', 'status-' + option.name.replace(/\s+/g, '-').toLowerCase())}>{option.name.toUpperCase()}</span>
    );
  };
  ...
  return (
    <div className="datatable-devices">
      <DataTable ref={dt} value={devices} resizableColumns columnResizeMode="fit" lazy
        ...
        <Column body={actionColumnBody} headerStyle={{width: '15em', textAlign: 'center'}}
